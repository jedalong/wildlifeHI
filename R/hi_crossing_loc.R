...
#'
#' @details
#'  This tool simply calculates where crossings occurs from tracking fixes in 
#'  relation to the input human infrastructure (assumed to be linear features).
...
#' See \code{?hi_get_osm}.
#' @param crs_code (optional) a CRS code to "project" data prior to performing intersection analysis.
...
#'
#' @return
#'  This function returns a \code{sf} POINT object containing the locations where a trajectory crosses 
#'  linear features (based on connecting consecutive GPS fixes)
#'
#' @examples
#' \dontrun{
#' data(fishers)
...
hi_crossing_loc <- function(move,osmdata,crs_code,...){
  ...
  id_df <- data.frame(trackId=sf_p1$trackId,
                      ...
                      timestamp1=sf_p1$timestamp,  #migth not be the best way to get times
